...
public class ProcessData {
	...
	private static void print_matrix() {
		...
		for (int i = 0; i < Main.table1NoOfAttrs; i++) {
			for (int j = 0; j < Main.table2NoOfAttrs; j++) {
				Main.resFile.writeToFile(Main.dataTypeCompatibilityMatrix[i][j] ? "T "
						: "F ");
	...
	public static void doStep3Generic() {
		...
		// print final results
		Main.resFile.writeToFile("\n\nFinal Join Pairs: count = " + globalFreqItemSetList.size() + "\n");
		Collections.sort(globalFreqItemSetList);
		
		if (globalFreqItemSetList.size() != 0) {
			for (ItemSet cur : globalFreqItemSetList) {
				Main.resFile.writeToFile(cur.toString() + "\n");
			}
		}
	...
	private static void calculateFrequentItemsets() {
		...
		for (int i = 0; i < candidateItemSetList.size(); i++) {
			...
			else
			{
				...
				do
				{
					supportCntArr[loopCnt] = getTargetScaledSupportGeneric(Main.table1Name, Main.table2Name,
							is.sourceList, is.targetList);
	...
	public static String getCommaSeperatedList(int tableId, ArrayList<Integer> list) {
		String ret = "";
		for (int cur : list)
			ret += gN(tableId, cur) + ",";
...
//			if(Main.flag == 1)
				support = support * (1 / Main.sampleValue);			
//			else
//			{	
//				int sampleSrcDistinctSize = new HashSet(sample1List).size();
//				
//				if (sampleSrcDistinctSize != 0)
//					support *= ((double)sampleSrcDistinctSize / completeSrcDistinctCount);
//			}
...
class ItemSet implements Comparable<ItemSet> {
	...
	public String toString() {
		return sourceList.toString() + " , " + targetList.toString()
				+ " ,Support = " + support;
	}
